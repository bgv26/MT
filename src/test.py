...
def get_rooms_areas(rooms, area):
    ...
    areas = [int(area * room / 100) for room in PROPORTIONS[rooms - 1][:-1]]
    areas.append(area - sum(areas))
    return ' '.join((str(int(x)) if x - int(x) == 0 else '{:.1f}'.format(x) for x in areas))
...
def from_bn_flats_and_rooms(item, sell=True):
    try:
        ...
        if object_type == 'квартира':
            ...
            rooms_num = get_node_value(item, 'rooms-total', True)
            info['rooms_num'] = rooms_num
        else:
            rooms_num = get_node_value(item, 'rooms-offer', True)
            info['rooms_num'] = '0'
        ...
        info['options_object_type'] = str(int(get_node_value(item, 'new-building')) + 1)

        if area_living:
            info['area_rooms'] = get_rooms_areas(int(rooms_num), float(area_living))
        else:
...
def to_cian_flats_and_rooms(root_node, data):
    ...
    options = etree.SubElement(offer, 'options', object_type=data['options_object_type'])
    if 'options_ipoteka' in data:
        options.set('ipoteka', data['options_ipoteka'])
